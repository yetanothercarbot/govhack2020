...
void setup(void) {
  ...
  datum1.purchaseTime = 1597463508;
  datum1.recycle = 5;
}
  ...
  if (success) {
    ...
    if (uidLength == 7)
    {
      ...
      for(int row = 0; row < 8; row += 2) {
        Serial.print("0x");
        Serial.print(toWrite[row], HEX);
        Serial.print("   ");
        Serial.print("0x");
        Serial.println(toWrite[row+1], HEX);
        Serial.print(toWrite[row], BIN);
        Serial.print("   ");
        Serial.println(toWrite[row+1], BIN);
      }
      ...
      for (uint8_t i = 4; i < 39; i++) {
        ...
        if(i < 13) {
          for(byte n = 0; n < 4; n++) {
            data[n] = (uint8_t) toWrite[i-4] & (0xF * (int)pow(10,n));
